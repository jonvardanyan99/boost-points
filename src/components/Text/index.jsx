...
export const Text = ({ type, children, className }) => {
  ...
  if (type === 'p1') {
    return <p className={classNames(styles.p1, className)}>{children}</p>;
  }

  if (type === 'p2') {
    return <p className={classNames(styles.p2, className)}>{children}</p>;
  }

  if (type === 'p3') {
    return <p className={classNames(styles.p3, className)}>{children}</p>;
  }

  if (type === 'p4') {
    return <p className={classNames(styles.p4, className)}>{children}</p>;
  }

  if (type === 'p5') {
    return <p className={classNames(styles.p5, className)}>{children}</p>;
  }

  if (type === 'p6') {
    return <p className={classNames(styles.p6, className)}>{children}</p>;
  }
